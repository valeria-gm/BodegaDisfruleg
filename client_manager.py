...
class ClientManagerApp:
    ...
    def add_client_dialog(self):
        ...
        tk.Label(name_frame, text="Nombre:", width=15, anchor="w").pack(side="left")
        ...
        tk.Label(type_frame, text="Tipo de Cliente:", width=15, anchor="w").pack(side="left")
        ...
        tk.Button(button_frame, text="Guardar", 
                 command=lambda: self.save_client(popup, name_entry.get(), phone_entry.get(), 
                                              email_entry.get(), type_var.get(), type_values, None), 
    ...
    def edit_client_dialog(self):
        ...
        tk.Button(button_frame, text="Guardar Cambios", 
                 command=lambda: self.save_client(popup, name_entry.get(), phone_entry.get(), 
                                              email_entry.get(), type_var.get(), type_values, client_id), 
                 bg="#4CAF50", fg="white", padx=10, pady=5).pack(side="left", padx=10)
        ...
        try:
            if client_id:  # Update existing client
                self.cursor.execute("""
                    ...
                    WHERE id_cliente = %s
                """, (name, phone if phone else None, email if email else None, type_id, client_id))
                ...
                self.cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s)
                """, (name, phone if phone else None, email if email else None, type_id))
